    ...
    with BME680() as bme680:
        ...
        while running:
            ...
            print(f"Raw Temp: {tph['temperature']:.2f} °C  "
                  ...
                  f"Comp Temp: {bsec['temperature']:.2f} °C  "
                  f"Comp Hum: {bsec['humidity']:.2f} %  "
                  f"IAQ: {bsec['iaq']:.1f}  "
                  f"sIAQ: {bsec['static_iaq']:.1f}  "
                  f"CO2: {bsec['co2_equivalent']:.1f} ppm  "
                  f"bVOC: {bsec['breath_voc_equivalent']:.3f} ppm  "
                  f"Gas%: {bsec['gas_percentage']:.1f}  "
                  f"CompGas: {bsec['compensated_gas']:.1f}  "
                  f"Stab: {bsec['stabStatus']:.0f}  "
                  f"RunIn: {bsec['runInStatus']:.0f}  "
                  f"Accuracy: {bsec['iaq_accuracy']}")
