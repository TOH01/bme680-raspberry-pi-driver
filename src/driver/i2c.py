...
class I2CDevice():
    def __init__(self, address: int, bus=1):
        self.bus = SMBus(bus)
    ...
    def _verify_address(self) -> None:
        try:
            ...
            raise RuntimeError(f"No I2C device at address {self.address:#04x}")
    ...
    def write_register_masked(self, register: Register, value: int) -> None:
        ...
        merged = [
            (c & ~m) | (r & m)
            for c, r, m in zip(current, raw, register.mask)
        ]
        self.write_at(register.start, merged)

    def read_register_group(self, register_group: RegisterGroup
                            ) -> dict[str, int]:

        start, size = register_group.get_range()
